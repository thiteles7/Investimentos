...
CLIENT_SECRET = "YOUR_GOOGLE_CLIENT_SECRET"     # Substitua pelo seu Client Secret
REDIRECT_URI = "http://localhost:8501"         # Altere conforme seu domínio
...
def google_login():
    ...
    if "code" not in params:
        ...
        st.markdown(f"Por favor, [faça login com sua conta do Google]({authorization_url})")
        st.stop()  # Para que a execução pare aqui até o callback
    else:
        code = params["code"][0]
        ...
        resp = oauth.get(USER_INFO_ENDPOINT)
        user_info = resp.json()
        st.session_state["user_info"] = user_info
        st.session_state["username"] = user_info.get("email")
        st.experimental_set_query_params()  # Limpa parâmetros de query
...
def main():
    ...
    # Se o usuário ainda não estiver autenticado, faz o login com Google
    if "user_info" not in st.session_state:
        st.write("### Autentique-se com o Google para acessar o app")
        google_login()
    ...
    if st.sidebar.button("Sair"):
        ...
        st.experimental_set_query_params()  # Limpa parâmetros de consulta
        st.experimental_rerun()
